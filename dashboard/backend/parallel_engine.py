        ...
        for wp in packages:
            ...
            for st in queued:
                ...
                await c.execute(
                    ...
                    json.dumps({"dispatcher": "safe_tick", "launch_mode": "claim_only"}, ensure_ascii=False),
                )
                await c.execute(
                    """
                    UPDATE subtasks
                ...
                claimed.append({"id": st["id"], "work_package_id": wp["id"], "agent_id": agent_id, "agent_instance_id": instance_id, "title": st["title"]})
                slots -= 1
    for item in claimed:
    ...
    async with pool.acquire() as c:
        ...
        counts = await c.fetchrow(
            ...
            SELECT COUNT(*) total,
                   ...
                   COUNT(*) FILTER (WHERE status IN ('running','queued','blocked')) open
