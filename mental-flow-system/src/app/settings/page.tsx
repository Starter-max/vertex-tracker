    ...
    <main className="min-h-screen bg-slate-950 px-4 pb-24 pt-6 text-slate-100">
      <div className="mx-auto max-w-3xl space-y-4">
        ...
        <div className="flex gap-3 flex-wrap">
          ...
          <a className="rounded-2xl border border-cyan-700 px-4 py-3 text-cyan-300" href="/report">Визуальный отчёт</a>
        </div>
        ...
        <label className="flex items-center gap-3 rounded-3xl border border-slate-800 bg-slate-900/70 p-4"><input type="checkbox" checked={state.settings.dailyReminderEnabled} onChange={e => setState({ ...state, settings: { ...state.settings, dailyReminderEnabled: e.target.checked } })} /> Ежедневное напоминание о разборе</label>
      </div>
      <BottomNav />
    </main>
  );
